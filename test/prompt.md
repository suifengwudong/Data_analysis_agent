...
*   **可视化动作**:
    *   **落点分布**: 
        *   使用 `r_plot_map` 绘制全球陨石分布图，颜色区分 `fall` 状态。观察是否存在“南极聚集效应”(Found 类) 以及落点是否集中在陆地。
        *   使用 `r_filter_data` 分别筛选出 Fell 类和 Found 类数据，单独绘制两张地图进行对比分析。
        *   使用 `r_visualize` (kde) 绘制纬度 (`reclat`) 分布的密度对比图 (x=reclat, color=fall)，在一张图中展示 Fell 与 Found 的纬度分布差异。
    *   **质量分布对比**:
...
*   **建模动作**:
    ...
    *   **公式**: `log10_mass ~ year * fall + reclat * fall`。
    *   **假设**: 验证是否“发现年份越晚，平均质量越小”，以及这种趋势在 Found 类（搜寻驱动）和 Fell 类（随机目击）中是否不同。
    *   **解读**: 检查 `fall` 的指示变量系数以及 `year:fall` 和 `reclat:fall` 的交互项系数，分析不同发现模式下的驱动因素差异。
